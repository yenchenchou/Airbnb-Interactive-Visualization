...
export default {
  ...
  methods: {
    ...
    update_plot(){
      ...
      // d3.select('body').select('#click_data_stats').remove();
      // d3.select('body').select('#click_data_stats')
      //   .text('Your Selection Price: NaN')
      var data = this.hotel;
      var priceAvg = d3.nest().key(d => d.properties.room_type)
        .rollup(function(v) {
            return {
      ...
      d3.select("#bar_g").append('g') 
        ...
        d3.select('body').select('#click_data_stats')
          ...
          .style('color', function(){
            if(this.incomingpoint.properties.room_type == 'Entire home/apt' &&
              this.incomingpoint.properties.price > 277){
                return '#FF5A5F'
            } else if(this.incomingpoint.properties.room_type == 'Entire home/apt' &&
              this.incomingpoint.properties.price <= 277){
              return '#00A699'
            } else if (this.incomingpoint.properties.room_type == 'Private room' &&
              this.incomingpoint.properties.price > 95){
              return '#FF5A5F'
            } else if (this.incomingpoint.properties.room_type == 'Private room' &&
              this.incomingpoint.properties.price <= 95){
              return '#00A699'
            } else if (this.incomingpoint.properties.room_type == 'Shared room' &&
              this.incomingpoint.properties.price > 55){
              return '#FF5A5F'
            } else if (this.incomingpoint.properties.room_type == 'Shared room' &&
              this.incomingpoint.properties.price <= 55){
              return '#00A699'
